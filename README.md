# Claude Code ハーネスエンジニアリング サンプルプロジェクト

**Claude Code のスキル・サブエージェント・スラッシュコマンドを活用したハーネスエンジニアリングのサンプル実装です。**

ドキュメント先行 → TDD 実装というワークフローを、Claude Code の各種ハーネス機能でどう自動化・標準化できるかを示すことが主目的です。
