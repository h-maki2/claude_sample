...
**重要**: 作成したいプロダクトの要求メモは、ユーザーが以下のファイルに保存する必要があります:

**ファイルパス**: `docs/ideas/<ビジネスサブドメイン名>initial-requirements.md`
...
`<英語名>` は、PRD作成プロセスのステップ0で `ubiquitous-finder` から取得したビジネスサブドメインの英語名です。
...
まず、ユーザーが作成した仕様メモを確認します:

```bash
Read('docs/ideas/<英語名>/initial-requirements.md')
```
